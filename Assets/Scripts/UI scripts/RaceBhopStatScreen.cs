...
public class RaceBhopStatScreen : MonoBehaviour
{
    ...
    public void updateStats()
    {
        ...
        //Timer!
        if(lastJumpAttempt.time <= currentJumpAttempt.time)
        {
            raceTime.text = currentJumpAttempt.time.ToString("F2") + " (" + lastJumpAttempt.time.ToString("F2") + "▲)";
            raceTime.color = Color.red;
        }
        else
        {
            raceTime.text = currentJumpAttempt.time.ToString("F2") + " (" + lastJumpAttempt.time.ToString("F2") + "▼)";
            raceTime.color = Color.green;
        }
    }
}
